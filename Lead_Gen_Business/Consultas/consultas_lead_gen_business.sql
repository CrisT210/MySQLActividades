...
-- 5.
SELECT s.domain_name AS sitio, COUNT(l.leads_id) AS nro_clientes_potenciales, l.registered_datetime
FROM sites AS s
...
-- 8. 
SELECT CONCAT(c.first_name, ' ', c.last_name) AS cliente, s.domain_name AS sitio, COUNT(l.leads_id) AS nro_clientes_potenciales, l.registered_datetime
FROM clients AS c
LEFT JOIN sites AS s ON c.client_id = s.client_id
LEFT JOIN leads AS l ON s.site_id = l.site_id
WHERE l.registered_datetime BETWEEN '2011/01/01' AND '2011/12/31'
GROUP BY s.site_id
ORDER BY c.client_id ASC, nro_clientes_potenciales DESC;
